...
is jwlc-specific (and gates the workflow). It's **set (2026-07-24), so push to
`master` auto-deploys** — this is live, not pending.
...
(Tailscale + the `coolify-token` helper), or just push to `master` (the var is
set → auto-deploy).
