...
// key static pages. URLs use the environment's own host (Astro.site).
...
// Listing statuses worth indexing — currently-marketed properties. Sold
// listings are excluded (their pages still resolve directly).
const INDEXABLE = new Set(['active', 'coming_soon', 'pending', 'under_contract']);

export const GET: APIRoute = async ({ site }) => {
  const origin = (site?.href ?? 'https://juliewrightlandcompany.com/').replace(/\/$/, '');
